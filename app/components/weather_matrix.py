...
def render_weather_matrix(weather):
    ...
    with matric_1: st.metric(label ="Temperature", value = temperature, border=True)
    with matric_2: st.metric(label = "Wind Speed", value = windspeed, border=True)
    with matric_3: st.metric(label = "Rainfall", value = rainfall, border=True)
    with matric_4: st.metric(label = "Weather Station", value = station, border=True)
