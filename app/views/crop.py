...
from app.services import get_crop_types, filter_data
from app.components import render_table, render_table_form, render_filter, render_graph_card, render_profit_trend, render_amount_trend, render_cost_revenue_trend
...
def load_nav():
    return st.segmented_control("Crop Nav Bar ", options=["Table", "Graph", "Trend"], default="Table", width="stretch", selection_mode="single", required=True, label_visibility="hidden")
...
def load_crop_graph_card(crops):
    ...
    for crop in crops:
        with col[curr_col]:
            render_graph_card(crop, graph_type="crop")
...
def load_crop_yield_trend(crops):   
    with st.container(border=True):
        ...
        st.divider()
        render_amount_trend(crops, show_table = True, trend_type="crop")
...
def load_crop_filter(crop_types, mode):
    ...
    filter_response = render_filter(crop_types, filter_type="crop", mode=mode) 
    if filter_response["error_code"]:
        ...
        if filter_response["data"]:
            ...
            filtered_crop = filter_data(Crop, st.session_state.user, *filter_response["data"])

        else:
            st.error("Filter: OFF")
            filtered_crop = filter_data(Crop, st.session_state.user, sort="Production Year")
...
def load_crop_form(crop_type_dict):
    ...
    if st.session_state.get("crop_form", False):
        render_table_form(crop_type_dict, menu = False, form_type="crop")

    elif st.session_state.get("table_menu", False):
        render_table_form(crop_type_dict, edit_item = st.session_state.edit_item, menu=True, form_type="crop")
